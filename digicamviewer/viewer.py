...
class EventViewer():
    ...
    def __init__(self, event_stream, camera_config_file, n_samples,
                 scale='lin', limits_colormap=None, limits_readout=None,
                 time_bin_start=0, pixel_id_start=0):
        ...
        self.flag = None
        self.readout_view_types = ['raw', 'baseline substracted', 'photon',
                                   'trigger input', 'trigger output',
                                   'cluster 7', 'reconstructed charge']
        ...
        self.trace_readout, = self.axis_readout.step(
            np.arange(self.n_samples) * 4, np.ones(self.n_samples), where='mid'
        )
        self.trace_time_plot, = self.axis_readout.plot(
            np.array([self.time_bin, self.time_bin]) * 4, np.ones(2),
            color='k', linestyle='--'
        )
    ...
    def compute_trace(self):
        ...
        if self.readout_view_type in self.readout_view_types:
            if self.readout_view_type == 'raw':
                print('trace:', self.adc_samples)
                image = self.adc_samples
            elif self.readout_view_type == 'trigger output' \
                    and self.trigger_output is not None:
                image = np.array([self.trigger_output[pixel.patch]
                ...
                image = np.array([trigger_input_patch[pixel.patch]
                                  for pixel in self.camera.Pixels])
            elif self.readout_view_type == 'photon' and \
                    self.dl1_container.pe_samples_trace is not None:
                image = self.dl1_container.pe_samples_trace
            elif self.readout_view_type == 'baseline substracted' and \
